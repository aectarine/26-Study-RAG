...
| 자동화 테스트 | 부분 완료 | `test_quality.py`의 핵심 질문·출처·거절 응답 검증 |
...
| 3단계 | 질문 유형별 테스트와 기대 출처 작성 | 높음 | 진행 중 | `test_quality.py` 기본 평가 케이스 작성 |
...
4. `test_quality.py`를 실행해 문서에 없는 질문의 거절 응답 검증
5. 질문 유형과 기대 출처를 추가해 품질 테스트 확장
